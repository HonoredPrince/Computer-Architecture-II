...
/*void multiplicacao_linear(Matrix& r, const Matrix& m1, const Matrix& m2) {
  long int B = 4;
  for (int jj = 0; jj < M_SIZE; jj = jj + B)
  for (int kk = 0; kk < M_SIZE; kk = kk + B)
  for (int i = 0; i < M_SIZE; ++i) {
    for (int j = jj; j < std::min(jj + B-1, M_SIZE); ++j) {
      float resultado = 0.0f;
      for (int k = kk; k < std::min(kk + B-1, M_SIZE); ++k) {
        const float e1 = m1.valores[i][k];
        const float e2 = m2.valores[k][j];
        resultado += e1 * e2;
      }
      r.valores[i][j] = r.valores[i][j] + resultado;
    }
  }
}*/
...
void multiplicacao_linear(Matrix& r, const Matrix& m1, const Matrix& m2) {
  for (int k = 0; k < M_SIZE; ++k)
    for (int i = 0; i < M_SIZE; ++i) {
      for (int j = 0; j < M_SIZE; ++j) {
        r.valores[i][j] = m1.valores[i][j] * m2.valores[i][j];
      }
    }
}
